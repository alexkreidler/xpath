...
describe('XPath library tests', () => {
  ...
  it('evaluate', () => {
    ...
    const doc = xpath.installDOM3XPathSupport(new dom().parseFromString(xml), new xpath.XPathParser());
    ...
    expect(nodes[0].toString()).to.equal('<title>Harry Potter</title>');
  });
  ...
  it('select text node', () => {
    ...
    expect(xpath.select('//title/text()', doc).toString()).to.equal('Harry,Potter');
  });
  ...
  it('select xpath with namespaces, prefixes different in xml and xpath, using a resolver', () => {
    ...
    expect(asNodes(xpath.selectWithResolver('//ns:title/text()', doc, resolver))[0].nodeValue).to.equal('Harry Potter');
    expect(
  ...
  it('string() with no arguments', () => {
    const doc = new dom().parseFromString('<book>Harry Potter</book>');
  ...
  it('string value of document fragment', () => {
    const doc = new dom().parseFromString('<n />');
  ...
  it('select1() on parsed expression', () => {
    ...
    expect(single.toString()).to.equal('<title>Harry Potter</title>');
  });
  ...
  it('work with nodes created using DOM1 createElement()', () => {
    const doc = new dom().parseFromString('<book />');
